...
const AnimeList = ({ api }) => {
  return (
    <div className="grid lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-4 grid-cols-3 gap-3 sm:gap-5 px-4">
      {api.data?.map((anime, index) => {
        return (
          <Link
            href={`/anime/${anime.mal_id}`}
            className={`cursor-pointer group flex-col bg-color-secondary rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 ease-out hover:-translate-y-1.5 border border-transparent hover:border-color-accent ${index >= 9 ? 'hidden sm:flex' : 'flex'}`}
            key={index}
          >
