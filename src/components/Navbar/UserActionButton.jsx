...
const UserActionButton = async () => {
  ...
  return (
    <div className="flex gap-2 justify-between items-center py-1">
        {
            user ? <Link href="/users/dashboard" className="bg-transparent text-white border border-white py-1 px-4 md:px-5 text-sm md:text-base inline-block rounded-full hover:bg-white hover:text-color-accent transition-all font-semibold">Dashboard</Link> : null
        }
